...
1. 交换两个变量的值
   ...
   ```
           ...
           // var num2 = temp;  //把temp的值取出放进nym2中
           // console.log(num1);
           // console.log(num2);
   ```
...
2. 不使用临时变量，交换两个数值变量的值
   ...
   ```
           ...
           // num1 = num1 - num2; //剪掉num2的20并赋进num1
           // //完成数字交换
...
Null  				一个表明 null 值的特殊关键字。 JavaScript 是大小写敏感的，因此 null 与 Null、NULL或变体完全						不同。	

Undefined 	和 null 一样是一个特殊的关键字，undefined 表示变量未定义时的属性。 

 Symbol  		代表 	( 在 ECMAScript 6 中新添加的类型)。一种实例是唯一且不可改变的数据类型。 

Object 			对象	 对象则称为合成类型（complex type）的值，因为一个对象往往是多个原始类型的值的合						成，可以看作是一个存放各种值的容器。  
...
- 字符串拼接
  ...
  ```javascript
  console.log(11 + 11);				//22	类型相同,直接相加减
  console.log('hello' + ' world');	//helloworld	两个字符串会拼接
  console.log('100' + '100');			//"100100"		两个字符串值会拼接
  console.log('11' + 11);				//"1111"  	类型不一样会拼接
  console.log('male:' + true);		//"male:true"  一个为字符串会拼接
  ```
...
- String()
  ...
  比如：undefined(没有赋值就是undefined)和null(空)
  ```
...
>可输入弹窗:  prompt("提示内容"); 
>
>如果输入内容需要转换为数字可以简写: var 变量 = parseInt(prompt("提示内容"));
